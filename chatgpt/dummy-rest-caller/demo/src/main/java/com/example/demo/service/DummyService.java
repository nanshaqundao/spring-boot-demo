...
//                .retryWhen(Retry.backoff(3, Duration.ofSeconds(3)).doBeforeRetry(System.out::println).filter(throwable -> !(throwable instanceof ClientException || throwable instanceof ServerException)));
    public Mono<String> getIsin() {
        var monoX = webClientWithTimeout.post().uri(uriBuilder -> uriBuilder.path("/api/isin").build()).body(BodyInserters.fromValue("dummy")).exchangeToMono(clientResponse -> {
            if (clientResponse.statusCode().is4xxClientError()) {
                System.out.println("4xx is not considered an error");
                return clientResponse.bodyToMono(String.class);
            } else if (clientResponse.statusCode().is5xxServerError()) {
                System.out.println("5xx");
                return clientResponse.bodyToMono(String.class).flatMap(body -> Mono.error(new ServerException("Server error: " + body)));
            } else {
                return clientResponse.bodyToMono(String.class);
            }
        }).doOnNext(result -> {
    ...
    private RetryBackoffSpec retrySpec =
            Retry.backoff(3, Duration.ofSeconds(3))
                    ...
                    .filter(
                            throwable -> !(throwable instanceof ClientException || throwable instanceof ServerException))
