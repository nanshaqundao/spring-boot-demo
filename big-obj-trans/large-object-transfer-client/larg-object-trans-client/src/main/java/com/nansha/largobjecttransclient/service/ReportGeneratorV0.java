...
/**
 * ReportGeneratorV0 is a service that fetches messages from the PublishingServiceClient and
 * generates a report. It fetches messages in pages and processes them, and then finalizes the
 * report.
 *
 * <p>This is the old version of the ReportGenerator service, which uses a reactive programming
 * model with Mono.
 */
@Service
public class ReportGeneratorV0 {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ReportGeneratorV0.class);

  public ReportGeneratorV0(PublishingServiceClient client) {
    this.client = client;
        ...
        messageStates -> {
          if (!messageStates.isEmpty()) {
            ...
            if (messageStates.size() == pageSize) {
              ...
              finalizeReport(); // Finalize the report if this page is not full, indicating it's the
                                // last page
            }
          } else {
  ...
  private void createReport(List<MessageState> messages) {
    ...
    for (MessageState message : messages) {
      reportBuilder
          .append(message.getFieldA())
          .append(",")
          .append(message.getFieldB())
          .append(",")
          .append(message.getFieldC())
          .append(",")
          .append(message.getFieldD())
          .append("\n");
    }
